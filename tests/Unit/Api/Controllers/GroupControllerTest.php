...
beforeEach(function () {
     ...
     */
    $this->transformGroup = function (object $group, array $expand = []): array {
...
it('returns the expected fields from transformGroup', function () {
    ...
    expect($result)
        ->toHaveKey('id', 1)
        ->toHaveKey('title', 'Test Group')
        ->toHaveKey('contribution_options')
        ->toHaveKey('meeting_ids')
        ->toHaveKey('updated')
        ->and($result['contribution_options']['has_options'])->toBeTrue();
});

it('expands meetings in transformGroup when requested', function () {
    ...
    expect($result)
        ->toHaveKey('meetings')
        ->not->toHaveKey('meeting_ids')
        ->and($result['meetings'])->toBeArray()
        ->and($result['meetings'][0]['id'])->toEqual(100);
});
